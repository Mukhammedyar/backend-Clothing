...
class ShoesController {     
  async getAll(req, res) {
    try {
      const product = await shoesModel
    .find();
  ...
  async create(req, res) {
    try {
      ...
      const product = await shoesModel.create(newPostData);
      res.status(201).json(product);
    } catch (error) {
      res.status(500).send(error.message);
    }
  ...
  async edit(req, res) {
    try {
      ...
      const updatedProduct = await shoesModel
    .findByIdAndUpdate(
        productId,
        updatedData,
  ...
  async delete(req, res) {
    try {
      const { productId } = req.params;
      const deletedProduct = await shoesModel
